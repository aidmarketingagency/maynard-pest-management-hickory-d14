(function(){
  // --- Sticky CTA: hide while real CTA section is in view ---
  ...
  // v2 spec (2026-07-24): SEPARATE playIO and rearmIO observers.
  // getBoundingClientRect() init-visible check required (guard gates on this).
  ...
  if ('IntersectionObserver' in window){
    // playIO: plays the thread when it enters the viewport
    ...
    // rearmIO: resets on full exit so next entry starts clean
    ...
    // "already visible on load" check (required: guard gates on getBoundingClientRect)
  ...
  // --- Animated stat counter: target $1,500, formats as "$1,500" ---
  // Re-runs on every scroll re-entry. Count-up stays under prefers-reduced-motion.
  var statEl = document.getElementById('statNumber');
  var statReplayBtn = document.getElementById('statReplayBtn');
  if (statEl && 'IntersectionObserver' in window){
    // Initialize DOM nodes (no innerHTML)
    statEl.textContent = '';
